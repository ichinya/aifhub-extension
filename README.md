...
- Planning from one explicit canonical GitHub issue uses that issue number as the stable plan identity: issue `#156` becomes OpenSpec change `156` or legacy sequential prefix `0156_`. Issue-derived plans persist the exact primary issue separately from secondary roadmap links and bind the creation branch to the numeric change; the active-change resolver checks that binding before slug matching. Ambiguous multi-issue input keeps ordinary allocation unless one issue is explicitly primary, and collisions fail closed instead of overwriting or allocating another ordinal. See [OpenSpec Compatibility](docs/openspec-compatibility.md).
