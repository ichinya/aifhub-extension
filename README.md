...
- Planning from one explicit canonical GitHub issue uses that issue number as the stable plan identity: issue `#156` becomes OpenSpec change `156` or legacy sequential prefix `0156_`. Ambiguous multi-issue input keeps ordinary allocation unless one issue is explicitly primary; collisions fail closed instead of overwriting or allocating another ordinal. See [OpenSpec Compatibility](docs/openspec-compatibility.md).
