...
- Adds optional read-only research delegation and comparison of materially different interfaces to explore/plan/improve. The coordinating parent reconciles source-bound findings; small tasks stay direct and existing confirmation/artifact rules remain in force. See [mattpocock adaptation](docs/mattpocock-adaptation.md) for the implemented scope.
- Supports bounded parallel implementation in host-managed Git worktrees through the existing execution helper: exact worker baselines, serial file integration, combined checks, durable recovery and closure before task progress. See [isolated execution](docs/isolated-execution.md) for host prerequisites and limits.
- Strengthens task design, difficult-bug reproduction, observable-interface tests, architecture assessment and focused review support through conditional shared guidance. Existing commands retain task identity, write scope, retry limits and one combined review verdict.
...
- Creates local `.gitignore` files in runtime directories during setup and writes, including configured paths; preserves existing ignore rules and keeps plans/specifications available to Git.
- Prototypes [SDD profiles and SessionBrief (P0)](docs/sdd-profiles.md) for OpenSpec: planning depth stays separate from required gates, `ai-factory aifhub-session-brief` compiles exact source revisions into runtime context, and implementation traces bind the consumed digest. [ADR 0004](docs/adr/0004-sdd-profiles-and-session-brief.md) describes the prototype; [ADR 0005](docs/adr/0005-ai-factory-plan-methodologies.md) defines the revised AI Factory plan ownership across methodologies and optional tools, which still needs runtime integration.
...
### AI Factory 2.19 Reviewed Release

AIFHub is source-reviewed against the published AI Factory `2.19.0` release — Git tag `v2.19.0` at commit [`e144f95c89de0d0cb6c9b38dbe8c2907b00dc5ce`](https://github.com/lee-to/ai-factory/commit/e144f95c89de0d0cb6c9b38dbe8c2907b00dc5ce), matching the npm `ai-factory@2.19.0` `gitHead` — while retaining the compatibility range `>=2.11.0 <3.0.0`. The earlier 2026-09-01 review covered the pre-release snapshot `3c1ddd4740d7b1c30d8ecb3dc80fa5e7b8d7ef5a` declaring `2.19.0`; the published release adds 8 further commits across 22 files, for a `2.18.1...2.19.0` range of 15 commits and 32 changed files. The consumer smoke now binds an exact published `2.19.0` executable through the `--v219-*` toolchain slot alongside retained `2.18.1` coverage.

The 2.19 delta is additive for AIFHub:
...
- `## Requirements Reconciliation` is an exact upstream compatibility token. AIFHub planning emits it conditionally in `proposal.md`, refinement preserves it verbatim, and implement/verify/fix treat it as the committed requirement-consistency record. AIFHub mirrors the conflict-stop semantics (`ERROR [requirement-conflict]`, `WARN`/`ERROR [requirement-ambiguity]`); the upstream `HANDOFF_MODE` `blocked_external` subagent protocol stays upstream-owned.
- `/aif-qa-check` gains upstream-owned browser replay evidence under `paths.qa/<branch-slug>/browser-replay/` (`TC-NNN.js` scripts plus `history/`), bound by case/target/script digests with a one-proof-run rule. These branch-scoped artifacts never satisfy AIFHub change-scoped verify, coverage, rules, done, or archive evidence.
...
- Planning from one explicit primary MCP work item supports GitHub, Linear, Jira, YouGile, and other providers, and uses its readable provider ID as the plan prefix: `156`, `ENG-431`, `PROJ-77`, and generic opaque-ID fallbacks become IDs such as `156-fix-login`, `eng-431-fix-login`, `proj-77-refresh-token`, or `yougile-a1b2c3d4-refresh-token`. Source-bound plans persist provider, full primary source, external ID, and creation branch separately from secondary roadmap links; ordinary plans omit that reserved section. The resolver checks one exact binding before slug matching, uses the current pointer to disambiguate several plans intentionally created on the same branch, and contains unrelated malformed bindings to warnings. Ambiguous multi-item input keeps ordinary allocation unless one item is explicitly primary, and collisions fail closed. See [OpenSpec Compatibility](docs/openspec-compatibility.md).
...
- `/aif-qa-check` consumes branch-scoped `test-cases.md` and writes branch-scoped `qa-check.md` plus upstream-owned `browser-replay/` scripts and history; these do not satisfy AIFHub verify, coverage, done, or archive gates. See [OpenSpec Compatibility](docs/openspec-compatibility.md).
...
The reviewed OpenSpec baseline is OpenSpec `1.13.0`, replayed sequentially from baseline `1.3.1` and including prerelease `1.6.0-beta.1`, while the compatible stable CLI range remains `>=1.3.1 <2.0.0`. The `1.13.0` checkpoint is bound independently to official Git tag `v1.13.0` at commit `9d4e5974e5c0d9a09b9c6c1e1eb0975e80ec4461` and to checksum-verified npm package `@fission-ai/openspec@1.13.0`; this is local exact-package evidence, not CI or production evidence. See [OpenSpec Compatibility](docs/openspec-compatibility.md) for the reviewed-release ledger, exact CLI matrix, and adapter-only ownership notes.

OpenSpec `validate --archived` is advisory-only. It is not part of package validation scripts, tracked CI, the shared runner's current-change validation argv, `/aif-verify`, `/aif-done`, or the release acceptance PASS boolean. The [1.13.0 audit](docs/openspec-1.13.0-audit.md) covers apply warnings, repeated sections, fenced coverage and archive preservation. The [1.12.0 audit](docs/openspec-1.12.0-audit.md) adds findings reports, advisory archive preflight and repository-grounded planning. The [1.11.0 audit](docs/openspec-1.11.0-audit.md) covers optional diff/batch reads, strict Purpose remediation, archive rename ordering, schema rollback, and Antigravity shared-root migration; those optional commands and generated tools remain upstream-owned.
...
| [OpenSpec Compatibility](docs/openspec-compatibility.md) | Optional CLI adapter policy, exact-tagged OpenSpec `1.13.0` reviewed baseline from `1.3.1`, pinned AI Factory `2.19.0` release, AI Factory 2.18 classic/ultra planning, research, verification, archive and ownership matrix, reviewed no-ops, and capability flags |
...
| [Agent Plugins 1.0 Compatibility Research](docs/agent-plugins-research/README.md) | Issue #200: verified two-skill research subset, explicit exclusions, bounded references, and native project-bound MCP probes; product export pending |
...
- The published AI Factory `2.19.0` release (tag `v2.19.0`, npm `ai-factory@2.19.0`) is an accepted update target alongside `2.18.1`; the consumer smoke binds exactly one published-executable target per run through the `--v218-*` or `--v219-*` toolchain slots.
- `ai-factory update --force` is the global exact `2.17.0`-to-target project refresh used by the consumer contract (`2.17.0` remains the update source for both `2.18.1` and `2.19.0` targets): it refreshes selected built-in skills and managed extensions, then reapplies injections. Stable `2.18.0` remains the ultra/transfer feature boundary.
...
- An installed AIFHub post-update handler automatically runs the fresh `aifhub-mode init --json` implementation after AIFHub is actually refreshed by `ai-factory update`, `ai-factory extension update`, or the targeted update above. This initializes enabled tools and missing runtime `.gitignore` files. Unchanged/skipped refreshes and updates targeting other extensions do not initialize AIFHub; a nonzero updater exit prevents initialization, and initialization failure makes the overall command fail. Executable coverage uses AI Factory 2.18.1 or 2.19.0.
- First transition from a build without the handler still needs one `ai-factory aifhub-mode init --json`: AI Factory loads extension commands before replacing their files, so a newly downloaded handler cannot run inside that first update process. Subsequent refreshes run initialization automatically. Initial `extension add` remains followed by the existing `/aif-analyze` setup workflow.
- `ai-factory upgrade` is the v1-to-v2 skill-name migration command; it is not the `2.17.0`-to-target update path.
- `npm test` includes an offline deterministic consumer harness with an injected exact `2.17.0` source executor and a selectable `2.18.1`/`2.19.0` target executor while retaining the public 2.18 smoke command and flag names. `npm run smoke:ai-factory-2-18 -- <explicit local toolchain arguments>` is a separate opt-in live driver: missing prerequisites are `NOT_RUN`, it never downloads a toolchain, and local success is not release, deployment, registry, or end-user migration proof.
