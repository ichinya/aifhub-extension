...
- Missing facts, conflicting requirements, or architecture uncertainty select `research` and return `/aif-explore` before implementation. `tracer` is reserved for P1; P0 must not claim a tracer run or promotion. Task text length and model confidence do not establish risk or completeness.
