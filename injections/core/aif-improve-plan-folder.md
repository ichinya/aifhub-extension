...
- Treat an existing `## AIFHub Source Binding` as reserved identity metadata. Preserve its complete heading and body byte-for-byte during ordinary refinement; `Primary issue` is immutable, and `Branch` may change only for an explicit branch-rebind request that keeps the same primary issue and passes source-binding validation.
