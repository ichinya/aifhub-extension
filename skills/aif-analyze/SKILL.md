...
- On a fresh config created with AI Factory 2.19 compatibility, include the upstream default `warmup.paths: []`. Treat `warmup.paths` as user-owned after creation: preserve an existing list byte-for-byte through structural mode patches, and do not backfill the section into an existing config when it is absent.
