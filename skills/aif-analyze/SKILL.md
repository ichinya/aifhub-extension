...
- After config is persisted, run `ai-factory aifhub-review-policy scaffold --json` as the only review-policy creation path. The shared resolver canonicalizes the real project root and nearest existing parent, rejects symlink/Windows junction components, hard-link targets, and canonical escapes, and blocks managed files plus canonical OpenSpec, project-rules, generated-rules, plan/spec, archive, runtime-state, and QA roots. Do not reproduce these checks with generic Read, Write, Edit, or shell path operations.
- Accept only a normalized project-relative Markdown file path that remains inside the project root. Reject absolute paths, URI-like values, paths that escape the project root, non-Markdown targets, directory targets, linked components, managed-file collisions, and protected-root descendants. Do not read or write a rejected target.
...
- The installed scaffold command uses [references/review-policy-template.md](references/review-policy-template.md) when the safe target is missing and creates it without overwriting a concurrent or existing file. Replace guidance comments later only when direct repository evidence supports them; preserve comment prompts for unresolved sections instead of inventing policy.
...
- The helper also creates missing runtime-local `.gitignore` files containing `*` and `!.gitignore` for state/QA in both modes and generated rules when OpenSpec is enabled. It covers configured runtime paths and fixed defaults still used by existing writers. Preserve existing ignore files; do not overwrite user exceptions, modify the root `.gitignore`, or untrack existing files. Keep reusable QA scenarios and reviewed permanent results in durable documentation or plan material rather than transient QA directories.
