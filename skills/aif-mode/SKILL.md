...
Setup also creates missing `.gitignore` files inside runtime state/QA/generated directories through `scripts/runtime-gitignore.mjs`, including configured runtime paths. The two rules are `*` and `!.gitignore`. Existing files are preserved, including user exceptions; the root `.gitignore` and Git index are unchanged. Dry-run previews missing ignore files without creating directories; status/doctor never repair them. Re-run `ai-factory aifhub-mode init --json` after updating an installed extension to initialize ignores in existing runtime directories.
