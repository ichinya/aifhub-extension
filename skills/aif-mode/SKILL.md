...
Setup also creates missing `.gitignore` files inside runtime state/QA/generated directories through `scripts/runtime-gitignore.mjs`, including configured runtime paths. The two rules are `*` and `!.gitignore`. Existing files are preserved, including user exceptions; the root `.gitignore` and Git index are unchanged. Dry-run previews missing ignore files without creating directories; status/doctor never repair them.

When the installed build contains `commands/post-update-init.mjs`, native `ai-factory update` and `ai-factory extension update [aifhub-extension]` automatically invoke the freshly installed init implementation after AIFHub files are refreshed. An unchanged/skipped refresh or another extension's targeted update does not trigger initialization. A nonzero updater exit prevents the hook from running; an init failure returns a nonzero exit without rolling back the update. For the first transition from a build without this handler, run `ai-factory aifhub-mode init --json` once: the updater loads the old command modules before downloading their replacements. Do not claim that first transition was initialized automatically.
