...
- **Reads**:
  - `.ai-factory/config.yaml` — project configuration and paths
  - `.ai-factory/RULES.md` — project-level rules (if present)
  - `.ai-factory/rules/base.md` — base rules created by `aif-analyze`
  - Plan-local `rules.md` — plan-specific rules (if active plan exists)
  - Changed files / current diff (via `git diff`)
...
1. Read `.ai-factory/config.yaml` for path configuration.
2. Load rules in priority order:
   - `.ai-factory/RULES.md` (project-level, highest priority)
   - `.ai-factory/rules/base.md` (base rules)
   - Plan-local `rules.md` (plan-specific, if active plan exists)
...
1. Check against applicable rules (project, base, plan-local).
...
- If rules are missing or outdated: suggest `/aif-analyze` or `/aif-rules`.
...
- If rules are outdated, suggest `/aif-rules` but do not edit them.
