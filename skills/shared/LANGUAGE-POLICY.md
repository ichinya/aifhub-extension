...
- `## Requirements Reconciliation` is an upstream-owned compatibility token parsed by downstream workflow checks. Keep the fixed English heading and preserve an existing section body verbatim except bounded synchronization updates by the owning refinement workflow, and never resolve a recorded conflict by rewriting the section; route conflicts to their owning workflow or the user.
