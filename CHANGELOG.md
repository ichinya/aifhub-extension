...
### Исправлено
- Для issue #156 `/aif-plan` теперь использует номер явно указанного GitHub issue как stable plan identity вместо очередного порядкового ID: OpenSpec-native change получает exact decimal ID, а legacy `sequential` — совместимый four-digit prefix. Отдельный `AIFHub Source Binding` сохраняет canonical primary issue и exact creation branch, legacy classic синхронизирует их в `status.yaml`, а active-change resolver выбирает binding до slug-эвристики и current pointer. Same-number cross-repository links, multi-issue ambiguity, malformed/mismatched binding, existing collisions, `9999` cap и `HANDOFF_BRANCH_PREPARED=1` обрабатываются детерминированно без overwrite, suffix или silent fallback allocation.
