...
- Before editing `proposal.md`, capture the exact byte ranges for `## Original Request` and existing `## Research Context`. Patch around those ranges: preserve the complete Original Request section byte-for-byte and preserve the committed Research Context snapshot plus source revision unless the user explicitly requested a research rebase.
