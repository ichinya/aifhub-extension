...
- Do not archive, do not write runtime state, and do not create legacy plan artifacts in OpenSpec-native mode.
- Return findings first, then a gate result: `PASS`, `PASS with notes`, or `FAIL`.
- Include active OpenSpec change, canonical artifacts inspected, generated rules state, runtime state path, QA evidence path, counts for blocking/important/optional findings, and next recommended command.
