...
- Use `## Original Request` only as the read-only raw intent anchor. Use an embedded `## Research Context` snapshot and its source revision as authoritative committed scope; canonical specs, design, tasks, and implementation remain the verification contract.
