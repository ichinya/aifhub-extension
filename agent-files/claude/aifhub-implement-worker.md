...
- A `replan_required` or `blocked` plan-compliance receipt returns remaining tasks to the planning owner; never write `plan-compliance.json` or treat it as QA evidence, and tracer artifacts never satisfy completion gates without an owner promote plus canonical updates.
...
- Use `## Original Request` only as the read-only raw intent anchor. Use an embedded `## Research Context` snapshot and its source revision as authoritative committed scope; do not mutate either section or silently apply newer research requirements. Treat an existing `## Requirements Reconciliation` as the committed requirement-consistency record and read its declared authority before editing; never mutate it.
...
- Before editing or accepting an in_progress assignment, reconcile the task's expected behavior against the authoritative requirements (canonical specs, `## Requirements Reconciliation`, `## Research Context`) and the `## Original Request` raw intent anchor. A clear unresolved conflict emits `ERROR [requirement-conflict]` and returns the task to the parent without edits; ambiguity that leaves the contract unclear emits `WARN [requirement-ambiguity]` and stops for clarification instead of guessing. When behavior depends on independent selectors, states, modes, or input shapes, confirm the task covers each applicable supported combination across validation, persistence, output/transition, and side effects.
...
For an isolated assignment, also read `skills/shared/ISOLATED-EXECUTION.md` from the installed AIFHub extension root. Work only in the parent's specified worktree and existing single-task run after group admission; use ordinary `resume`, `checkpoint` and `result` there. Return the exact result digest/version and stop editing. The parent owns local acceptance, `isolation-*` transfers and combined verification in its integration root. Do not create worktrees, switch roots, commit, accept results, synchronize canonical checkboxes, clean up a worktree or dispatch nested workers.
