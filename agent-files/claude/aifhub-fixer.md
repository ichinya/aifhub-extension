...
- Persist `qaEvidenceRead`, `regressionCheck`, `preFixResult`, `postFixResult`, and `fallbackDecision` through `writeFixTrace()` under `.ai-factory/state/<change-id>/fixes/`. Record exit codes or observed statuses and fallback reasons without credentials, tokens, cookies, authorization values, sensitive URL parameters, or raw provider output.
