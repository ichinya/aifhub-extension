  ...
  return JSON.stringify({
    ...
    sources: {
      ...
      openspec: {
        ...
        reviewedStableVersions: ['1.3.1', '1.4.0', '1.4.1', '1.5.0', '1.6.0', '1.7.0', '1.8.0', '1.9.0', '1.10.0', '1.11.0', '1.12.0', '1.13.0'],
        reviewedPrereleaseVersions: ['1.6.0-beta.1'],
        lastSync: '2026-09-01',
        optional: true,
        requiresNode: '>=20.19.0',
        mode: 'optional-cli-adapter',
        notes: 'Validated against upstream OpenSpec 1.13.0; AIFHub remains adapter-only.'
      }
    },
    ...extra
  });
...
describe('validate-extension.mjs', () => {
  ...
  it('keeps the OpenSpec 1.13.0 manifest, runner and immutable compatibility constraints atomic', async () => {
    ...
    assert.equal(openspec.version, '1.13.0', 'sources.openspec.version');
    assert.equal(OPENSPEC_LATEST_REVIEWED_VERSION, '1.13.0', 'runner reviewed baseline');
  ...
  it('fails when OpenSpec metadata advances without the runtime reviewed-version diagnostic', async () => {
    const parsed = JSON.parse(validAifhubMetadata());
    parsed.sources.openspec.version = '1.13.1';
    parsed.sources.openspec.reviewedStableVersions.push('1.13.1');
    await writeValidProject({ metadata: JSON.stringify(parsed) });
