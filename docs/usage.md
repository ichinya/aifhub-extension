...
An explicit `ultra` request with missing, malformed, prerelease, unsupported `<2.18.0`, or mismatched CLI/project version evidence stops before writes and suggests the regular profile. A valid legacy ultra bundle is handled atomically; invalid marker/phase shapes and classic/ultra collisions fail closed.
...
| Explicit ultra rejected | AI Factory version evidence is missing, prerelease, `<2.18.0`, malformed, or mismatched to project provenance. | Use the regular plan/research profile or provide a matching stable local 2.18+ project toolchain; no ultra artifact was written. |
