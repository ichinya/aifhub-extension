...
`Primary source` is the only collision identity. `Provider` and `External ID` make the binding readable but cannot authorize reuse alone. A secondary reference in `Roadmap Linkage.Issues`, including the same external ID from another provider or repository, cannot replace the primary binding. The exact attached creation branch maps the prefixed change back to downstream `/aif-improve`, `/aif-implement`, and `/aif-verify`; this binding is checked before ordinary slug branch variants. After successful creation, planning also writes the complete derived change ID to the current-change pointer. A malformed, duplicate, ambiguous, or prefix-mismatched binding fails closed.
