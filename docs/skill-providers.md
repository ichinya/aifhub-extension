[Предыдущая страница](context-providers.md) | [К документации](README.md) | [Следующая страница](safety-providers.md)
...
| AIFHub review/security/verify parity | `EXECUTED(la_only_lifecycle_proxy, non_promotable)` | Один полный 32-case прогон [lifecycle proxy](skill-providers-research/ponytail-lifecycle-ab/results.md) на `omniroute/la/ornith-1.5-35b-a3b` (`pi 0.84.4`, 4 repetitions per arm, hidden graders): review и security 4/4 в обеих руках, verify и fix 0/8 с одинаковыми contract-discipline отказами; корректность между руками не изменилась (8/16 vs 8/16), Ponytail добавил только token overhead. Другие модели и полный lifecycle envelope не запускались; promotion evidence по-прежнему отсутствует |
