...
| Paired Pi implementation proxy | `EXECUTED(mixed_non_promotable)` | [Two complete 24-case runs](skill-providers-research/ponytail-pi-ab/results.md) pin `pi 0.84.4`, `omniroute/lq/qwen3.8-27b` and `omniroute/la/ornith-1.5-35b-a3b`, three real-project commits, four repetitions per arm and hidden graders. Qwen moved from 6/12 baseline PASS to 8/12 with Ponytail; LA moved from 11/12 to 9/12 raw, with one LA provider-error pair excluded from capability comparison. Go improved, Laravel regressed, and the proxy does not support promotion |
...
[Ponytail Pi A/B Scenarios](skill-providers-research/ponytail-pi-ab/README.md) реализуют первый implementation-only proxy этого контракта: три exact-commit копии реальных TypeScript, Go и Laravel/PHP проектов, четыре парных повтора на task, два Pi model envelope и независимые hidden graders. [Выполненные 48 строк](skill-providers-research/ponytail-pi-ab/results.md) показали model- и stack-dependent результат: на 23 сопоставимых парах baseline прошёл 16, Ponytail — 17, но Go улучшился с 4/8 до 7/8, а Laravel ухудшился с 4/7 до 2/7. Единственная provider-error пара исключена, а efficiency считается только по both-pass парам. Набор намеренно не запускает AIFHub lifecycle sidecars, поэтому policy остаётся `manual_experiment_only`.
