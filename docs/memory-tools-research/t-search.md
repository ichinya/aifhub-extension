...
From a search-quality perspective, Recall@10 improved in three scenarios, stayed unchanged in two, and regressed in one. Precision more than doubled, but remained only `0.183333`, corresponding to an average false-positive rate of `0.816667`; the result set is still noisy. Ranking quality is the clearest gain: five of six candidate rankings placed a relevant chunk first, raising reciprocal rank to `0.833333`. Because T-Search used 26 iterative searches versus six baseline calls, this supports agentic query planning and ranking over a single direct `rg` search on this six-task sample, not a general claim of consistently superior retrieval.
