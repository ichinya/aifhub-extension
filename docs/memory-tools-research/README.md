...
- `tencentdb-agent-memory` — `conditional_external_runtime_only` (issue #201): recall user-owned Gateway предлагать только explicit opt-in для research-follow-up средних/больших en-репо и impl-continuity самых больших; install/setup/purge остаются user-owned, npm `postinstall` патчит host runtime — AIFHub не устанавливает и не запускает; см. [research](tencentdb-agent-memory.md) и [benchmark](tencentdb-agent-memory-benchmark-results.md);
- `understand-anything` остаётся `reject_defer`: exact `v2.9.0` interactive skill lifecycle не прошёл confinement eligibility, поэтому AIFHub не устанавливает и не запускает provider; [research](understand-anything.md) и [benchmark](understand-anything-benchmark-results.md) разделяют non-promotable synthetic reviewed-output contract и actual provider provenance;
- `t-search` остаётся `reject_defer`: exact Q4_K_M прошёл 6/6 локальных synthetic pairs и поднял Recall@10 `0.667→1.0`, но ухудшил FPR `0.44→0.495`, потребовал 206,212 tokens и был примерно в 790 раз медленнее `rg`; это всё ещё source-only agentic planner/ranker поверх отдельного user-owned index и endpoint, а normal AIFHub workflows не скачивают weights, не запускают stack и не сохраняют raw snippets/transcripts; см. [research](t-search.md) и [benchmark](t-search-benchmark-results.md);
...
| Understand Anything reviewed output | `ua-luna-low-20260722-r3` | 16/16 runner rows, 8 pair identities; 11/16 correctness PASS, 0/16 privacy PASS | [understand-anything-benchmark-results.md](understand-anything-benchmark-results.md) | Answer-independent `gpt-5.6-luna`/`low` matrix proved candidate context use 8/8 but full raw traces failed privacy and were purged; provider lifecycle/provider-generated matrix remain `NOT_RUN`/`SKIPPED`, policy `reject_defer`. |
| T-Search | `local_gguf_q4_reduced_context` | 6/6 paired synthetic rows | [t-search-benchmark-results.md](t-search-benchmark-results.md) | Exact Q4_K_M + pinned harness/llama.cpp: all safety gates PASS, Recall@10 `0.667→1.0`, FPR `0.44→0.495`, 449.815 s / 206,212 tokens vs 0.569 s `rg`; `pilot_negative`, external index lifecycle `NOT_RUN`, policy `reject_defer`. |
...
| context-mode Codex | `context-mode-134-luna-low-20260728` | 18/18 dry-run, 0 executed | [context-mode-benchmark-results.md](context-mode-benchmark-results.md#codex-plugin-re-evaluation-v10169--2026-07-28) | Exact `v1.0.169`: snapshot PASS; MCP/hooks BLOCKED by runtime self-install; plugin `NOT_RUN(auth_isolation_unavailable)`; no token claim. |
| context-mode Codex | authorized live follow-up `2026-08-03` | small + >1 MiB + continuity probes | [context-mode-benchmark-results.md](context-mode-benchmark-results.md#authorized-live-follow-up-v10169--2026-08-03), [live-authorized-evidence.json](context-mode-codex-ai-tester/live-authorized-evidence.json) | MCP recovered truncated tail facts but used `+120.2%` tokens; small case `+376.8%`; plugin failed nested shell interception; continuity not proven. |

For `t-search`, a non-promotable local synthetic source-retrieval pilot is recorded: 6/6 pairs and all local safety gates passed, but worse aggregate FPR and extreme latency/token overhead produced `pilot_negative`; a real-repository external-index lifecycle remains `NOT_RUN`, and upstream web-search results are not promotion evidence. No such benchmark is expected for `codex-agent-mem` and `agent-memory`, because they are not source retrieval tools. The Python/OpenSpec matrix includes negative/not-applicable selector rows for `codex-agent-mem`, but these validate policy only. `rohitg00-agentmemory` имеет authored non-promotable safety scenario: isolated standalone profile прошёл 4/4 rows и 2/2 pairs, но обе пары дали `avoid`; full-product runtime остаётся `NOT_RUN`, а decision — `reject_default`. Это не positive source-retrieval evidence и не разрешение normal tool runs. `codex-mem` и `eagle-mem` также отклонены до positive benchmark из-за неприемлемой scoped read/purge/default privacy safety.
...
| [context-mode](context-mode.md) | [results](context-mode-benchmark-results.md) | [mksglu/context-mode](https://github.com/mksglu/context-mode) | historical runtime `1.0.151`; static/live-isolated `v1.0.169` | Temporary output/context indexing and optional Codex plugin surface. | MCP conditional only for large truncating output, with no token-saving claim; tested plugin path avoid; `rg` remains baseline. |
...
| [T-Search](t-search.md) | [results](t-search-benchmark-results.md) | [t-tech/T-Search](https://huggingface.co/t-tech/T-Search), [harness](https://github.com/turbo-llm/t-search-harness) | exact Q4_K_M SHA-256 `f645dce…b8761bb`; harness `997a0ba1685d24ad840e3e2542b59952ff3fb362`; llama.cpp b10068 | Agentic query planning/ranking over a separately supplied index. | `reject_defer`; reduced synthetic pilot was `pilot_negative`; no MCP or production index lifecycle, raw snippets/transcripts forbidden. |
...
- `T-Search` не предлагать и не выбирать: local synthetic pilot уже показал recall gain, но одновременно FPR/latency/token regression; до promotion нужны real-repository bounded user-owned index/search lifecycle, redaction/freshness/purge и улучшение этого same-run tradeoff против `rg`; raw snippets/transcripts нельзя сохранять.
