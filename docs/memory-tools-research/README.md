...
- `t-search` остаётся `reject_defer`: это source-only agentic query planner/ranker поверх отдельного user-owned index и model endpoint, а не готовый retrieval provider; AIFHub не скачивает weights, не запускает harness/endpoints, не индексирует corpus, не probes/selects candidate и не сохраняет raw snippets/transcripts; см. [research](t-search.md) и [benchmark](t-search-benchmark-results.md);
...
| T-Search | static upstream evaluation | 0 paired AIFHub rows | [t-search-benchmark-results.md](t-search-benchmark-results.md) | Harness lint/type/tests pass in UTF-8 mode, but model run and AIFHub-like paired retrieval remain `NOT_RUN`; upstream fixed-web Recall@10 is author-reported context only, policy `reject_defer`. |
...
| [T-Search](t-search.md) | [results](t-search-benchmark-results.md) | [t-tech/T-Search](https://huggingface.co/t-tech/T-Search), [harness](https://github.com/turbo-llm/t-search-harness) | pinned BF16/FP8/NVFP4/GGUF revisions; harness `997a0ba1685d24ad840e3e2542b59952ff3fb362` | Agentic query planning/ranking over a separately supplied index. | `reject_defer`; source-only compound stack, no MCP/index lifecycle or AIFHub-like paired benchmark, raw snippets/transcripts forbidden. |
...
- `T-Search` не предлагать и не выбирать: до promotion нужны bounded user-owned index/search lifecycle, redaction/freshness/purge и same-run mixed code/docs/OpenSpec benchmark против `rg`; raw snippets/transcripts нельзя сохранять.
