...
Hook trust bypass был явно разрешён только для audited pinned snapshot внутри disposable test sandbox. Текущий replay contract требует exact envelope field `hook_trust_mode: test_only_pinned_snapshot_bypass`; отдельный boolean и default generated step не авторизуют bypass. Это test-only разрешение не применяется к normal AIFHub commands или user-owned installations.
