...
`context-mode` не входит в generic memory-tool matrix и не поддерживает `--preinitialize-tool`: любая explicit `--tool`/`--preinitialize-tool` list с `context-mode` атомарно отклоняется до создания output directory и возвращает structured JSON с `status: NOT_RUN`, `reason: context_mode_requires_dedicated_harness`, `tool_id: context-mode` и путём dedicated harness. Generic tools из такой mixed list намеренно не запускаются, чтобы explicit request не превращался в silent partial success. Для issue `#134` используйте только pinned isolated `scripts/context-mode-codex-ai-tester-{matrix,adapter,run,results}.mjs` harness.
