...
2. [Usage](usage.md) - полный command flow, `/aif-mode` switching and sync, AI Factory 2.18 mode matrix, regular/ultra research, classic/marked-ultra verification and archive boundaries, upstream project utilities, rules/review/security gates, finalization tail, update-not-upgrade guidance, deterministic/live consumer smoke, troubleshooting и examples.
...
6. [Context Loading Policy](context-loading-policy.md) - Base Context including the optional project glossary, review policy, optional Graphify/CodeGraph/Context7 guidance, GitHub-aware roadmap evidence, ownership boundaries, generated rules, quality gates, parent-owned commit grouping, upstream architecture/docs/QA/archive/distillation utilities, commit handoff и legacy path rules.
7. [Project Review Policy](review-policy.md) - configurable root-default `REVIEW.md`, scaffold ownership, review-only consumers, precedence, path safety и policy/session-state boundary.
8. [OpenSpec Compatibility](openspec-compatibility.md) - optional CLI adapter support, exact-tagged OpenSpec 1.9.0 reviewed baseline from 1.3.1, tracked Git/npm custody summary, AI Factory 2.18.1 reviewed baseline with cumulative 2.18 classic/ultra plan and research contracts, upstream coherence ownership, revision-bound receipt, archive guard, ownership/no-op ledger, artifact sync points, rules gate, Node requirements, validation policy flags и degraded mode.
9. [OpenSpec Artifact Validation](openspec-validation.md) - AIFHub contract validator поверх OpenSpec CLI validation.
10. [OpenSpec Coverage Matrix](spec-coverage.md) - requirement-to-task-to-code coverage evidence и verify/done policy.
11. [Legacy Plan Migration](legacy-plan-migration.md) - если существующие `.ai-factory/plans` artifacts нужно перенести в OpenSpec-native changes.
12. [Active Change Resolver](active-change-resolver.md) - active change selection, runtime paths, current pointer behavior и ambiguity diagnostics.
13. [Handoff Validation Profile](handoff-validation-profile.md) - read-only orchestration summary contract.
14. [ADR 0001](adr/0001-openspec-native-artifact-protocol.md) - v1 artifact ownership decision.
15. [ADR 0002: Optional Project Glossary](adr/0002-optional-project-context-glossary.md) - protocol-neutral glossary ownership, lexical authority and deferred OKF decision.
16. [ADR 0003: Durable Project Review Policy](adr/0003-durable-project-review-policy.md) - review-policy namespace, ownership, authority and session-state separation.
...
| Guide | Назначение |
|---|---|
| [Usage](usage.md) | Полный OpenSpec-native command flow, AI Factory 2.18 mode/profile matrix, regular/ultra research, revision-bound legacy-ultra receipt, archive/update boundaries, optional providers, gates, finalization tail и examples |
...
| [OpenSpec Compatibility](openspec-compatibility.md) | CLI adapter policy, exact-tagged OpenSpec 1.9.0 reviewed baseline from 1.3.1, tracked Git/npm custody summary, AI Factory 2.18.1 reviewed baseline, cumulative 2.18 plan/research/archive adapters, upstream coherence ownership, exact consumer ownership/no-ops, validation policy flags, sync points, rules gate, version support и degraded mode |
...
- AI Factory 2.18.1 reviewed baseline, including the upstream-owned Research Coherence Gate, immutable Original Request, canonical ultra depth in OpenSpec-native mode, marker-first legacy classic/ultra routing, regular/ultra research, revision-bound legacy-ultra verification receipt, OpenSpec-native archive guard, update-not-upgrade semantics, exact namespaced-agent handoffs, and explicit `/aif-loop`/`/aif-transfer` no-ops
...
- OpenSpec 1.9.0 reviewed baseline from 1.3.1, including `1.6.0-beta.1`, exact Git/npm custody, strict/non-strict task-numbering semantics, scenario-loss validation, archive serialization, rootless command behavior, advisory-only archived validation, native `skip_specs`, explicit capability retirement, nested task progress, archive instructions, numeric-leading IDs and nested specs, plus adapter-only ownership boundaries for Command Code, schemas, OpenSpec skills, `/opsx:*`, agent targets, tool integrations, Stores, workspace beta state, package management, and `openspec update`; OpenSpec 1.10/1.11 remain outside scope
