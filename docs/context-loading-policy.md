...
Canonical local lifecycle linkage comes from the proposal's standardized `## Roadmap Linkage` fields: `Issues`, `Milestone`, `Roadmap item/slice`, and `Rationale`. MCP work-item plan identity is separate: `## AIFHub Source Binding` stores exactly one `Provider`, `Primary source`, `External ID`, and exact creation `Branch`; secondary roadmap references and equal external IDs never substitute for that full primary binding. The active-change resolver checks exact source bindings before slug branch variants. Explicit `none` values are preserved and no command may infer missing linkage from remote metadata. `/aif-roadmap check` registers a linked active change as local `planned` and preserves or registers `finalized` only when durable done/archive evidence supports that state.
